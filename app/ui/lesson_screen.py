...
class _ExerciseController:
    def __init__(self, page: ft.Page, state: AppState, exercise: Exercise) -> None:
        ...
        self._hint_index = 0
        self._passed = False
        self._next_in_category_id: Optional[str] = None
    ...
    def build_view(self) -> ft.View:
        ...
        example_card = self._card("Example", [
            make_read_only_code_block(exercise.example_code.strip(), scale=self.scale),
        ])
    ...
    def _build_code_card(self) -> ft.Control:
        ...
        self.editor = make_code_editor(exercise.starter_code.strip(), scale=self.scale)
    ...
    def _build_output_card(self) -> ft.Control:
        ...
        self.details_text = ft.Text("", size=self._fs(12), font_family="Consolas", color="#D4D4D4", selectable=True)
        self.details_container = ft.Container(
            content=self.details_text, bgcolor="#1E1E1E", border_radius=8, padding=12, visible=False,
        )
    ...
    def _on_success(self) -> None:
        ...
        next_in_category = self.state.exercise_engine().next_unlocked_in_category(
            self.exercise.category, completed_ids,
        )
        self._next_in_category_id = next_in_category.id if next_in_category else None
        self.next_button.visible = next_in_category is not None
