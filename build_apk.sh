#!/usr/bin/env bash
# Builds an Android APK from the current source tree. `flet build` drives a
# real Flutter SDK + Android SDK/NDK under the hood; see
...
#
# Android is not this app's primary target (it's a desktop refresher tool
# first), but the Flet UI has no desktop-only dependencies, so a build is
# free to keep working if anyone wants it on a tablet/phone.
set -e
...
# on every build so the APK's Android versionCode advances every time,
# independent of pyproject.toml's [project].version (which only changes on
# an intentional release bump).
...
# Read [project].version out of pyproject.toml so the APK filename and the
# --build-version passed to Flutter can never drift from what's declared there.
...
# --module-name main is explicit (it also happens to be flet build's own
# default here) so this keeps working unambiguously if a second entry
# point is ever added, the way python-adventure-kids needed it.
# --build-number/--build-version set the Android versionCode/versionName.
"$FLETEXE" build apk --module-name main --yes \
